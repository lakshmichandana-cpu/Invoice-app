...
export class InvoiceManageComponent implements OnInit, OnDestroy {
  ...
  initForm() {
    ...
    if (!isNaN(this.invIdx)) {
      ...
      partyName = this.currentInvoice.partyName;
      clientMobile = this.currentInvoice.clientMobile;
      ...
      for (let i = 0; i < this.currentInvoice.items.length; i++) {
        itemList.push(
          new FormGroup({
            ...
            gst: new FormControl(
              this.currentInvoice.items[i].gst.toString(),
              Validators.required
            ),
    ...
    this.invoiceForm = new FormGroup({
      ...
      partyName: new FormControl(partyName, Validators.required),
      clientMobile: new FormControl(clientMobile, [
        Validators.required,
      ]),
  ...
  calculateTotal(priceControl: any, quantityControl: any, gstControl: any, tot_price1Control: any) {
    if (priceControl.valid && quantityControl.value) {
      let q = +quantityControl.value;
      let p = +priceControl.value;
      let g = +gstControl.value;
      let total = p * q;
      let tot_price = (total * g / 100) + total ;
      console.log("tot_price", tot_price)
      tot_price1Control.setValue(tot_price.toString());
    }
  }
  ...
  onSubmit(isDraft: boolean) {
    ...
    if (!this.invoiceForm.invalid) {
      ...
      let partyName = this.invoiceForm.get('partyName')?.value;
      let clientMobile = this.invoiceForm.get('clientMobile')?.value;
      let status = '';
      if (this.editMode) {
      ...
      let newItems = items.map((item: any) => {
        ...
        allCost += total;
        return { name, quantity, gst, price, total };
      });
